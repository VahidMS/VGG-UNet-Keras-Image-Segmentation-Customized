#@title Data Loader { form-width: "10%" }
#Original Code From: keras_segmentation/data_utils/data_loader.py
...
#gets a list of pairs of (img_url, manual_url)
...
#Gets the image numpy array and normalises it
def get_image_array(image_input,
                    width, height,
                    imgNorm="sub_mean", ordering='channels_first'):#image normalization
    """ Load image array from input """

    if type(image_input) is np.ndarray:
        # It is already an array, use it as it is
        img = image_input
    elif isinstance(image_input, six.string_types):#if image_input is an image url
    ...
    if imgNorm == "sub_and_divide":
        ...
        img[:, :, 0] -= 103.939 #these means are based on dogs and cats dataset, maybe I should change them
        img[:, :, 1] -= 116.779
        img[:, :, 2] -= 123.68
        img = img[:, :, ::-1] #The : means "take everything in this dimension" and the ::-1 means "take everything in this dimension but backwards."
                              #Your matrix has three dimensions: height, width and color. Here you're flipping the color from BGR to RGB. 
    elif imgNorm == "divide":
        img = cv2.resize(img, (width, height))
        img = img.astype(np.float32)
        img = img/255.0
...
#Gets a 2 channel segmentation 2D or 1D arrays
def get_segmentation_array(image_input, nClasses,
    ...
    if not no_reshape:#if no_reshape = False=>reshape=True!
